...
NS_SL_BEGIN
  ...
  bool VulkanCommon::PrepareVulkan( WindowParameters parameters ) {
    Window = parameters;
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanCommon::CreateInstance() {
    ...
    if( vkCreateInstance( &instance_create_info, nullptr, &VulkanBase.Instance ) != VK_SUCCESS ) {
...
#define VK_INSTANCE_LEVEL_FUNCTION( fun )                                                   \
    if( !(fun = (PFN_##fun)vkGetInstanceProcAddr( VulkanBase.Instance, #fun )) ) {              \
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanCommon::CreatePresentationSurface() {
    ...
    if( vkCreateWin32SurfaceKHR( VulkanBase.Instance, &surface_create_info, nullptr, &VulkanBase.PresentationSurface ) == VK_SUCCESS ) {
      return true;
    }
  ...
  bool VulkanCommon::CreateDevice() {
    ...
    if( (vkEnumeratePhysicalDevices( VulkanBase.Instance, &num_devices, nullptr ) != VK_SUCCESS) ||
        (num_devices == 0) ) {
    ...
    if( vkEnumeratePhysicalDevices( VulkanBase.Instance, &num_devices, &physical_devices[0] ) != VK_SUCCESS ) {
      std::cout << "Error occurred during physical devices enumeration!" << std::endl;
      return false;
    }
    ...
    for( uint32_t i = 0; i < num_devices; ++i ) {
      if( CheckPhysicalDeviceProperties( physical_devices[i], selected_graphics_queue_family_index, selected_present_queue_family_index ) ) {
        VulkanBase.PhysicalDevice = physical_devices[i];
      }
    }
    ...
    if( vkCreateDevice( VulkanBase.PhysicalDevice, &device_create_info, nullptr, &VulkanBase.Device ) != VK_SUCCESS ) {
      std::cout << "Could not create Vulkan device!" << std::endl;
      return false;
    }

    VulkanBase.GraphicsQueue.FamilyIndex = selected_graphics_queue_family_index;
    VulkanBase.PresentQueue.FamilyIndex = selected_present_queue_family_index;
    return true;
  }
  ...
  bool VulkanCommon::CheckPhysicalDeviceProperties( VkPhysicalDevice physical_device, uint32_t &selected_graphics_queue_family_index, uint32_t &selected_present_queue_family_index ) {
    ...
    for( uint32_t i = 0; i < queue_families_count; ++i ) {
      vkGetPhysicalDeviceSurfaceSupportKHR( physical_device, i, VulkanBase.PresentationSurface, &queue_present_support[i] );
...
#define VK_DEVICE_LEVEL_FUNCTION( fun )                                                   \
    if( !(fun = (PFN_##fun)vkGetDeviceProcAddr( VulkanBase.Device, #fun )) ) {                \
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanCommon::GetDeviceQueue() {
    vkGetDeviceQueue( VulkanBase.Device, VulkanBase.GraphicsQueue.FamilyIndex, 0, &VulkanBase.GraphicsQueue.Handle );
    vkGetDeviceQueue( VulkanBase.Device, VulkanBase.PresentQueue.FamilyIndex, 0, &VulkanBase.PresentQueue.Handle );
    return true;
  }
  ...
  bool VulkanCommon::CreateSwapChain() {
    ...
    if( VulkanBase.Device != VK_NULL_HANDLE ) {
      vkDeviceWaitIdle( VulkanBase.Device );
    }

    for( size_t i = 0; i < VulkanBase.SwapChain.Images.size(); ++i ) {
      if( VulkanBase.SwapChain.Images[i].View != VK_NULL_HANDLE ) {
        vkDestroyImageView( GetDevice(), VulkanBase.SwapChain.Images[i].View, nullptr );
        VulkanBase.SwapChain.Images[i].View = VK_NULL_HANDLE;
      }
    }
    VulkanBase.SwapChain.Images.clear();

    VkSurfaceCapabilitiesKHR surface_capabilities;
    if( vkGetPhysicalDeviceSurfaceCapabilitiesKHR( VulkanBase.PhysicalDevice, VulkanBase.PresentationSurface, &surface_capabilities ) != VK_SUCCESS ) {
      std::cout << "Could not check presentation surface capabilities!" << std::endl;
      return false;
    }

    uint32_t formats_count;
    if( (vkGetPhysicalDeviceSurfaceFormatsKHR( VulkanBase.PhysicalDevice, VulkanBase.PresentationSurface, &formats_count, nullptr ) != VK_SUCCESS) ||
        (formats_count == 0) ) {
    ...
    if( vkGetPhysicalDeviceSurfaceFormatsKHR( VulkanBase.PhysicalDevice, VulkanBase.PresentationSurface, &formats_count, &surface_formats[0] ) != VK_SUCCESS ) {
      std::cout << "Error occurred during presentation surface formats enumeration!" << std::endl;
      return false;
    }

    uint32_t present_modes_count;
    if( (vkGetPhysicalDeviceSurfacePresentModesKHR( VulkanBase.PhysicalDevice, VulkanBase.PresentationSurface, &present_modes_count, nullptr ) != VK_SUCCESS) ||
    ...
    if( vkGetPhysicalDeviceSurfacePresentModesKHR( VulkanBase.PhysicalDevice, VulkanBase.PresentationSurface, &present_modes_count, &present_modes[0] ) != VK_SUCCESS ) {
      std::cout << "Error occurred during presentation surface present modes enumeration!" << std::endl;
      return false;
    }
    ...
    VkSwapchainKHR                old_swap_chain = VulkanBase.SwapChain.Handle;
    ...
    VkSwapchainCreateInfoKHR swap_chain_create_info = {
      ...
      VulkanBase.PresentationSurface,                   // VkSurfaceKHR                   surface
    ...
    if( vkCreateSwapchainKHR( VulkanBase.Device, &swap_chain_create_info, nullptr, &VulkanBase.SwapChain.Handle ) != VK_SUCCESS ) {
      std::cout << "Could not create swap chain!" << std::endl;
      return false;
    }
    if( old_swap_chain != VK_NULL_HANDLE ) {
      vkDestroySwapchainKHR( VulkanBase.Device, old_swap_chain, nullptr );
    }

    VulkanBase.SwapChain.Format = desired_format.format;

    uint32_t image_count = 0;
    if( (vkGetSwapchainImagesKHR( VulkanBase.Device, VulkanBase.SwapChain.Handle, &image_count, nullptr ) != VK_SUCCESS) ||
        (image_count == 0) ) {
      std::cout << "Could not get swap chain images!" << std::endl;
      return false;
    }
    VulkanBase.SwapChain.Images.resize( image_count );

    std::vector<VkImage> images( image_count );
    if( vkGetSwapchainImagesKHR( VulkanBase.Device, VulkanBase.SwapChain.Handle, &image_count, &images[0] ) != VK_SUCCESS ) {
      std::cout << "Could not get swap chain images!" << std::endl;
      return false;
    }

    for( size_t i = 0; i < VulkanBase.SwapChain.Images.size(); ++i ) {
      VulkanBase.SwapChain.Images[i].Handle = images[i];
    }
    VulkanBase.SwapChain.Extent = desired_extent;
  ...
  bool VulkanCommon::CreateSwapChainImageViews() {
    for( size_t i = 0; i < VulkanBase.SwapChain.Images.size(); ++i ) {
      VkImageViewCreateInfo image_view_create_info = {
        ...
        VulkanBase.SwapChain.Images[i].Handle,          // VkImage                        image
      ...
      if( vkCreateImageView( GetDevice(), &image_view_create_info, nullptr, &VulkanBase.SwapChain.Images[i].View ) != VK_SUCCESS ) {
  ...
  VulkanCommon::~VulkanCommon() {
    if( VulkanBase.Device != VK_NULL_HANDLE ) {
      ...
      if( VulkanBase.SwapChain.Handle != VK_NULL_HANDLE ) {
        vkDestroySwapchainKHR( VulkanBase.Device, VulkanBase.SwapChain.Handle, nullptr );
      }
      vkDestroyDevice( VulkanBase.Device, nullptr );
    }

    if( VulkanBase.PresentationSurface != VK_NULL_HANDLE ) {
      vkDestroySurfaceKHR( VulkanBase.Instance, VulkanBase.PresentationSurface, nullptr );
    }

    if( VulkanBase.Instance != VK_NULL_HANDLE ) {
