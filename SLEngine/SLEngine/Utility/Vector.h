...
public:
     ...
     */
    explicit Vector<T>(unsigned int capacity)
    : _data()
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for cocos2d::Vector<T>!");
...
//    }
    ...
    unsigned int getIndex(T object) const
    {
    ...
    T getRandomObject() const
    {
        if (!_data.empty())
        {
            unsigned int randIdx = RandomHelper::random_int<int>(0, static_cast<int>(_data.size()) - 1);
     ...
     */
    void insert(unsigned int index, T object)
    {
    ...
    {
        unsigned int idx1 = getIndex(object1);
        unsigned int idx2 = getIndex(object2);
    ...
    void replace(unsigned int index, T object)
    {
        CCASSERT(index >= 0 && index < size(), "Invalid index!");
        CCASSERT(object != nullptr, "The object should not be nullptr");
